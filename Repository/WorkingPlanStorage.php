...
final class WorkingPlanStorage
{
    private const PREFIX = 'working_plan_';

    private string $fallbackStorage;

    public function __construct(private readonly Connection $connection, string $dataDirectory)
    {
        $this->fallbackStorage = rtrim($dataDirectory, '/') . '/working-plans.json';
    }
    ...
    public function loadByProjectId(int $projectId): ?array
    {
        ...
        try {
            $value = $this->connection->fetchOne('SELECT value FROM kimai2_demo_working WHERE name = :name', ['name' => $name]);
    ...
    public function saveByProjectId(int $projectId, array $rows): void
    {
        $name = self::PREFIX . $projectId;
        $payload = json_encode([
            'rows' => $rows,
        ...
        try {
            $id = $this->connection->fetchOne('SELECT id FROM kimai2_demo_working WHERE name = :name', ['name' => $name]);

            if ($id === false || $id === null) {
                $this->connection->insert('kimai2_demo_working', [
                    'name' => $name,
                    'value' => $payload,
                ]);
            ...
            $this->connection->update('kimai2_demo_working', ['value' => $payload], ['id' => (int) $id]);
        } catch (\Throwable) {
